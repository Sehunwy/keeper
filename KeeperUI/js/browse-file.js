...
var util = $.getUtil('/KeeperUI/js/util.js'),interface = $.getUtil('/KeeperUI/js/interface.js');
...
broFile.prototype = {
	browseFile: function(){
        ...
        browseFileDialog = $.loongDialog({
            "title": doI18n("keeper_file_browse_select_path"),
            ...
            "buttons": [{
                ...
                "callback": function(){
                    ...
                    if(ret.hasError){
                        ...
                        if(pathCurrent == ""){
                            $.loongDialog({
                                "content": doI18n("keeper_file_browse_no_select_path"),
                                "isModal": false,
                                "msgType": 'warning',
                            });
                            return;
        ...
        $browseFileForm.find(".file-path").attr("placeholder",doI18n("keeper_file_browse_placeholder_path"));
        $browseFileForm.find(".file-search").loongQuery({
            isInput: true,
            inputWidth: "290px",
            placeholderTxt: doI18n("keeper_search_folder"),
    ...
    initFileTable: function(path,curPageSize,keyword){
        ...
        if (tableDom) {
            ...
            tableDom = $browseFileForm.find("div[name=file-table]").loongTable({
                ...
                fields: [{
                    "name" : "name",
                    "title": doI18n("keeper_file_browse_file"),
                    "width" : "460px",
                    "overflowEllipsis" : true
                },{
                    "name" : "size",
                    "title": doI18n("keeper_file_browse_size"),
                    "width" : "120px",
                    ...
                    "name" : "creatTime",
                    "title": doI18n("keeper_file_browse_create_time"),
                    "formater": function(v, all){
                        if (v === 0) {
                            return "";
                        } else {
    ...
    initTableEvent: function(data){
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            if (data[i].folder) {
                if (data[i].isWorm) {
                    $(img).attr("src", "/KeeperUI/image/director-worm@2x.png");
                } else {
                    $(img).attr("src", "/KeeperUI/image/director@2x.png");
                }
            } else {
                $(img).attr("src", "/KeeperUI/image/file@2x.png");
            }
    ...
    initPageEvent: function(){
        ...
        $browseFileForm.find("div[name=create-folder]").on("click", function(){
            var createFolderDialog = $.loongDialog({
                "title": doI18n("keeper_file_browse_create_folder"),
                        ...
                        util.validateForm($createFolderForm, function(){
                            ...
                            $.ajax({
                                ...
                                util.dealAjaxError(data, null, function(){
                                    $.loongDialog({
                                        "content": doI18n("keeper_file_browse_operation_success"),
                                        "isModal": false,
                                        "msgType": "success"
                                    });
                                    self.initFileTable(pathCurrent);
                    ...
                    interface.queryPathExist({
                        ...
                        util.dealAjaxError(data, null, function(){
                            if (data.data.data) {
                                ...
                                $.loongDialog({
                                    "content": doI18n("keeper_file_browse_path_not_exists"),
                                    "isModal": false,
                                    "msgType": "error"
                                });
                            }
